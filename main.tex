...
\author{R.V.S.S Varun\\EE23BTECH11220}
\date{}
...
\begin{align}
    \omega_{p1} &= 2\pi\frac{F_{p1}}{F_s} = 0.26 \pi\\
...
\end{align}
\begin{align}
    \omega_{s1} &= 2\pi\frac{F_{s1}}{F_s} = 0.253 \pi\\
     \omega_{s2} &= 2\pi\frac{F_{s2}}{F_s} = 0.327 \pi\\
\end{align}
\end{enumerate}
...
$\Omega_{p1} =0.4327$, $\Omega_{p2} =0.5498$ and $\Omega_{s1} = 0.4197$, $\Omega_{s2} = 0.5642$
respectively.
...
\end{equation}
where $\Omega_0 = \sqrt{\Omega_{p1}\Omega_{p2}} = 0.488$ and $B = \Omega_{p2} - \Omega_{p1} = 0.1171$.

Substituting $\Omega_{s1}$ and $\Omega_{s2}$ in \eqref{eq:freq_transform} we obtain the stopband edges of lowpass filter 
\begin{align}
    \Omega_{Ls1} &= \frac{\Omega_{s1}^2 - \Omega_0^2}{B\Omega_{s1}} = -1.261\\
    \Omega_{Ls2} &= \frac{\Omega_{s2}^2 - \Omega_0^2}{B\Omega_{s2}} = 1.213
\end{align}
And we choose the minimum of these two stopband edges
\begin{align}
    \Omega_{Ls} = \mbox{min}(\vert \Omega_{Ls_1}\vert,\vert \Omega_{Ls_2}\vert) = 1.213.
\end{align}
...
\begin{lstlisting}
https://github.com/Varunryaliiith/Filter_Design/blob/main/codes/plot1.py
\end{lstlisting}
\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/plot1.png}
\caption{The Analog Low-Pass Frequency Response for $0.280  \leq \epsilon \leq 0.61$}
...
\begin{lstlisting}
https://github.com/Varunryaliiith/Filter_Design/blob/main/codes/sk_gen.c
\end{lstlisting}
...
\begin{lstlisting}
https://github.com/Varunryaliiith/Filter_Design/blob/main/codes/plot_zero_plt.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\begin{align}
    H_{a,LP}(s_L) &= \frac{G_{LP}}{\brak{s_L-s_7}\brak{s_L-s_8}\brak{s_L-s_9}\brak{s_L-s_10}\brak{s_L-s_11}\brak{s_L-s_12}}
\end{align}
